...
import { Video, Loader2, Save, Upload, CheckCircle, Play, RefreshCw, Home } from "lucide-react";
import { addHomeVideoServer } from "./actions";
...
export default function HomeVideoPage() {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            formData.append("folder", "home_videos");
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await addHomeVideoServer({
                imageId: publicId,
                mediaType: "video",
            });
            alert("Home video updated and saved successfully!");
        } catch (error) {
    ...
    return (
        <div className="max-w-xl mx-auto py-12 px-4">
            <div className="text-center mb-10">
                ...
                <h1 className="text-4xl font-black text-white tracking-tight mb-3">Home Video</h1>
                <p className="text-slate-400 font-medium">Upload and set the featured video for the home page.</p>
            </div>
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden backdrop-blur-xl">
                ...
                <div className="absolute -bottom-20 -left-20 w-64 h-64 bg-emerald-500/10 blur-[80px] rounded-full" />
                ...
                <div className="space-y-8 relative z-10">
                    <div 
                        ...
                        className={`aspect-video rounded-[2rem] border-2 border-dashed flex flex-col items-center justify-center text-center cursor-pointer transition-all relative overflow-hidden group
                            ${isUploading ? 'border-blue-500/50 bg-blue-500/5' : 
                              previewUrl ? 'border-blue-500/40 bg-black shadow-2xl' : 
                    ...
                    >
                        ...
                        {previewUrl ? (
                                ...
                                {!isUploading && !isSaving && (
                                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all bg-black/40 backdrop-blur-sm">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <span className="text-[10px] text-white font-black">Change Video</span>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="p-8 flex flex-col items-center">
                                <div className="h-16 w-16 bg-slate-900 rounded-2xl flex items-center justify-center mb-6 text-zinc-500 group-hover:text-blue-500 group-hover:scale-110 transition-all border border-slate-800 shadow-lg">
                                    <Video className="h-8 w-8" />
                                </div>
                                <div className="text-sm text-zinc-300 font-bold mb-1">Select Home Video File</div>
                                <div className="text-[10px] text-zinc-600 font-black">MP4, MOV or WEBM</div>
                            </div>
                        )}
                    </div>
                    ...
                    {publicId && !isSaving && (
                        <div className="flex items-center justify-center gap-2 text-blue-500/80 animate-in fade-in slide-in-from-bottom-2 duration-700">
                            <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                            <span className="text-[10px] font-black">Ready to save to database</span>
                        </div>
                    )}
                </div>
            </div>
